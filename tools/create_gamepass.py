...
async def create(session, game_id, price):
    async with session.post("https://apis.roblox.com/game-passes/v1/game-passes", data={"Name": "Gamepass", "Description": "Jo e x Is HeRe ", "UniverseId": game_id}, ssl=False) as response:
        if response.status == 200:
            data = await response.json()
